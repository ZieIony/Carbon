...
public class FlagsScrollView extends FrameLayout {
    ...
    private boolean inChild(int x, int y) {
        for (int i = 0; i < getChildCount(); i++) {
            ...
            boolean result = !(y < child.getTop() - scrollY
    ...
    private int getScrollRange() {
        ...
        Stream.of(getViews()).forEach(v -> {
            scrollRange[0] += v.getHeight() + ((MarginLayoutParams) v.getLayoutParams()).topMargin + ((MarginLayoutParams) v.getLayoutParams()).bottomMargin;
        });
        return Math.max(0, scrollRange[0] - getPaddingTop() - getPaddingBottom() - getHeight());
    }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        Stream.of(getViews()).forEach(v -> {
            int topMargin = ((MarginLayoutParams) v.getLayoutParams()).topMargin;
            v.layout(v.getLeft(), topMargin + height[0], v.getRight(), topMargin + v.getHeight() + height[0]);
            height[0] += v.getHeight() + topMargin + ((MarginLayoutParams) v.getLayoutParams()).bottomMargin;
        });
